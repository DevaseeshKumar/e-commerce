                ...
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                    <div className="flex justify-between items-center h-16">
                        ...
                        <Link to="/user/dashboard" className="flex items-center gap-2.5 group">
                            ...
                            <span className="text-xl font-bold text-gray-900 dark:text-white tracking-tight">ShopEasy</span>
                        </Link>
                    ...
                    {mobileOpen && (
                        <div className="md:hidden pb-4 space-y-1 border-t border-gray-100 dark:border-gray-700 pt-3">
                            {navLinks.map(link => (
                                <Link key={link.to} to={link.to} className="block px-4 py-2.5 text-sm font-medium text-gray-700 dark:text-gray-200 hover:bg-gray-50 dark:hover:bg-gray-700 rounded-lg" onClick={() => setMobileOpen(false)}>{link.label}</Link>
                            ))}
                            <Link to="/profile" className="block px-4 py-2.5 text-sm font-medium text-gray-700 dark:text-gray-200 hover:bg-gray-50 dark:hover:bg-gray-700 rounded-lg" onClick={() => setMobileOpen(false)}>Profile</Link>
                            <button onClick={() => { setMobileOpen(false); setLogoutModal(true); }} className="w-full text-left px-4 py-2.5 text-sm font-medium text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-lg">Log Out</button>
                        </div>
                    )}
                </div>
            </nav>
